...
**Algae** 是一个基于 Qt 的水藻生态策略游戏。玩家需要在有限的网格中合理布局不同类型的藻类，通过管理资源、利用藻类特性，提升整体产量，达成通关目标。游戏兼具策略性与趣味性.
>>>>>>> 3bf62900b99826016b0c195cd10829fbf7eb89d2
...
---
=======
不起眼的藻类贡献了绝大多数的二氧化碳吸收和大多数的氧气制造。本游戏模拟了多种藻类的共生与竞争，帮助玩家理解生态系统中资源分配、遮光效应、协同增益等机制。
...
- 友好的图形化界面，支持全屏
- 背景音乐与多种音效
- 鼠标悬浮查看格子详细信息（所有显示和判定均以本格实际光照为准）
- ESC 菜单支持暂停、重开、设置音量
...
- **鼠标悬浮**：查看格子当前资源、光照、可否种植等信息（所有显示和判定均以本格实际光照为准）
...
## 机制一致性说明
- 所有格子的高亮、可否种植、产量等判定，均严格依赖本格实际光照，与行列位置无关。同一列L值一致时显示和判定完全一致。
...
- **所有显示和判定均以本格实际光照为准，彻底消除"同一列下方格子L值高却不能种植"的问题。**
...
**Q1：为什么有些格子显示"光照不足"无法种植？**  
A：当前格子的光照低于所选藻类的"种植所需光照"阈值，通常是因为上方有遮光藻类或本格本身光照较低。所有显示和判定均以本格实际光照为准。

**Q2：为什么同一列下方格子L值高却显示不同？**  
A：不会出现此问题。所有显示和判定均以本格实际光照为准，同一列L值一致时显示和判定完全一致。
